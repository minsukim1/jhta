...
-- 최근 1주일 이내에 이 책을 구매한 사용자를 조회하기
-- 사용자아이디, 사용자명, 책제목, 구매수량, 구매날짜가 출력되어야 함.
select A.user_id, A.user_name, C.book_title, B.order_amount, B.order_registered_date
from sample_book_users A, sample_book_orders B, (select rownum ranking, book_title, book_price, book_no
                                                 from (select book_title, book_price, book_no
                                                 from sample_books
                                                 order by book_price desc)
                                                 where rownum <=3) C
where A.user_id = B.user_id
and B.book_no = C.book_no
and order_registered_date >= sysdate-7;
...
-- 급여를 기준으로 내림차순 정렬해서 순번을 부여했을 때 최고 급여를 받는11~20위에해당하는 직원의
-- 순번, 아이디, 이름, 급여를 조회하기
...
on sample_book_orders (order_price*order_amount);
