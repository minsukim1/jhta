...
public class StringDemo1 {
	...
	public static void main(String[] args) {
		...
		System.out.println("3번째 부터 9번째 앞까지를 포함하는 문자열 : "+ s2);
		...
		// 문자열에서 지정된 문자열(old)을 새문자열(nw)로 반환한다.
		// String replaceAll(String regexp, String replacement)
		// 문자열에서 지정된 정규표현식과 일치하는 문자열을 찾아서 새 문자열로 반환한다.
		...
		// 문자열을 지정된 표현식(혹은 구분문자)으로 자른 새로운 문자열로 반환한다.
		...
		// 문자열에 지정된 문자열이 포함되어 있으면 true를 반환한다.
		// int indexOf(String str)
		// 문자열에 지정된 문자열이 있으면 그 위치를 반환한다. 없으면 -1을 반환한다.
		// int lastIndexOf(String str)
		// 문자열에 지정된 문자열이 마지막으로 등장하는 위치를 반환한다. 없으면 -1을 반환한다.
		...
		System.out.println("문자열이 휴가신청서로 시작되는가? "+ b7);
		System.out.println("ms워드 파일인가? "+ b8);
		
		// int compareTo(String other)
		// 다른 문자열과 사전순서를 기준으로 비교했을 때, 같으면 0, 빠르면 음수, 늦으면 양수를 반환한다.
		...
		// String trim()
		// 문자열에서 불필요한 좌우공백을 제거한 새로운 문자열을 반환한다.
		String str9 = "                  안녕       나는       로봇이야              ";
		String s7 = str9.trim();
		System.out.println("공백제거 전 ["+str9+"]");
		System.out.println("공백제거 후 ["+s7+"]");
		
		// String String.valueOf(다양한 값)
		// 전달받은 값을 문자열로 변환해서 반환한다.
		int num1 = 123456;
		int num2 = 7890;
